<p align="center">
  <a href="https://aligilani.com/">
    <img alt="Ali Gilani Banner" src="./assets/banner.png" width="100%" />
  </a>
</p>

<h1 align="center">Hi, I'm Ali Gilani</h1>
<p align="center"><b>Software Engineer | Machine Learning | Data</b></p>

<p align="center">
  <a href="mailto:contact@aligilani.com"><img src="https://img.shields.io/badge/Email-contact%40aligilani.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email Ali Gilani" /></a>
  <a href="https://aligilani.com"><img src="https://img.shields.io/badge/Website-aligilani.com-0A66C2?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website Ali Gilani" /></a>
</p>

---

### About Me
- I build intelligent software systems that combine product thinking with practical engineering.
- My focus sits at the intersection of `software`, `machine learning`, and `data`.
- I enjoy turning ideas into reliable, production-ready solutions.

### What I Work On
- End-to-end web applications and platform features
- Applied machine learning projects and data workflows
- API design, integrations, and backend performance

### Tech Stack
<p>
  <img src="https://skillicons.dev/icons?i=python,typescript,javascript,react,nextjs,nodejs,fastapi,docker,postgres,mongodb,git,github,vscode&theme=dark" alt="Tech stack icons" />
</p>

### Current Focus
- Building scalable, intelligent systems
- Improving MLOps and deployment workflows
- Writing clean, maintainable code with strong DX

### Connect
- Website: [aligilani.com](https://aligilani.com)
- Email: [contact@aligilani.com](mailto:contact@aligilani.com)

---

<p align="center">Building intelligent systems, one project at a time.</p>
